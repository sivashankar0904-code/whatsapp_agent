...
// endpoint, and CRUD over /chats. There is no auth — it binds to all
// interfaces because it is meant to be reached from other containers on the
// same docker network, not from the internet. Do not publish this port on a
// host interface without adding authentication first.
func New(msgStore *messages.Store, chatStore *chats.Store) *gin.Engine {
	r := gin.Default()
